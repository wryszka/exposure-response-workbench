...
| R6 | **Robust ingestion of the real open feeds** the client's analyst struggled to pull (FIRMS/NASA fire + flood + storm) — solving the **API-timeout / blocking** pain. | **PARTIAL** | Framework with exponential-backoff/retry + Delta cache; **MeteoAlarm windstorm is LIVE and keyless** (verified: real Météo-France warning → 195 threatened props). FIRMS (fire) + Copernicus EFAS/EMS (flood) adapters built, running on **frozen real-shape samples**; go live with two free keys (`MAP_KEY`, `CDS_API_KEY`, see `docs/FEEDS.md`). |
